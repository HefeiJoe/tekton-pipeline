...
import warnings, base64, json, configparser, os, sys
import requests
...
# 批量导入SRPFrame的功能
def new_imports_SRPFrame(grafana_url, auth,srpFrame_data):
    try:
        url = "%s/api/frame/imports" % grafana_url
        headers = {"Authorization": auth, "Content-Type": "application/json"}
        res = requests.post(url=url, json=srpFrame_data.get('orgContent'), headers=headers)
        print(res.text)
    except Exception as e:
        print(e)
...
# 覆盖插件操作
def overwrite_plugin(grafana_url, token, uuid):
    try:
        url = "%s/api/file/overwrite" % grafana_url
        headers = {"Authorization": token,
                   # "Content-Type": "multipart/form-data; boundary=----WebKitFormBoundaryohc4o3fKVVV18fkn"
                   }
        data = {
            "Content-Type": "multipart/form-data; boundary=----WebKitFormBoundaryohc4o3fKVVV18fkn",
            'overwriteFile': (None, uuid)
        }
...
# 导入插件
def ImportPlugin(grafana_url, token, file, overwrite):
    try:
        url = "%s/api/file/up" % grafana_url
        headers = {"Authorization": token}
        files = {
        ...
        if response.status_code == 200:
            ...
            # elif 'uuid' in res
                ...
                if overwrite.lower() == "true":
                    res = overwrite_plugin(grafana_url, token, uuid)
                    print("Plugin already covered")
                    return res
                elif overwrite.lower() == "false":
                    print("The plugin already exists (not overwritten)")
                    return res
                else:
                    print("Please check for coverage")
        elif response.status_code == 401:
            print("You do not have permission to access the requested data or object! ")
        else:
...
if __name__ == '__main__':
    ...
    config_type = config.get("target", "type").lower()
    ...
    if options.srpframereplace != "":
        ...
        #config_srpframe_replace_array = [[0, 0], [0, 0], [0, 0], [0, 0]]
        i = 0
        for replace in config_srpframe_replace_list:
            ...
            i = i + 1
        # config_datasource_replace_array = [['https://dashboard-ifactory-adviiot-ifactory-ifactory.wise-paas.com',
        #                                     'https://dashboard-${cluster_namespace}-${cluster_cluster}.${externaldomain}'],
        #                                    ['https://api-srpalert.wise-paas.com/oeeen', 0], [0, 0], [0, 0]]
    ...
    if options.file != None and options.file != '':
        ...
        with open(options.file, 'r',encoding='utf-8') as f:
            try:
                ...
                elif (options.type != None) and (options.type == 'file'):
                    files = options.file
                    print(filename)
                    response = ImportPlugin(grafana_url, auth, files, options.overwrite)

                elif(options.type != None) and (options.type == 'srpframes'):
                    # TODO: 新增参数增加的部分
                    srpframe_data = f.read()
                    if srpframe_replace_count != 0:
                        for i in range(0, srpframe_replace_count):
                            srpframe_data = srpframe_data.replace(config_srpframe_replace_array[i][0],
                                                                  config_srpframe_replace_array[i][1])
                    srpframe_data_json = json.loads(srpframe_data)
                    new_imports_SRPFrame(grafana_url, auth,srpframe_data_json)
                            ...
                            if srpframe_replace_count != 0:
                                for i in range(0, srpframe_replace_count):
                                    srpframe_data = srpframe_data.replace(config_srpframe_replace_array[i][0],
                                                                          config_srpframe_replace_array[i][1])
                                    srpframe_data_json = json.loads(srpframe_data)
                            response = new_imports_SRPFrame(grafana_url, auth, srpframe_data_json)
                    except Exception as e:
                        print(e)
                if (options.type != None) and (options.type == 'file'):
                    files = filepath
                    print(file)
                    res = ImportPlugin(grafana_url, auth, files, options.overwrite)
